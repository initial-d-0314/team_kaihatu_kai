<x-app-layout>
    <div class="w-full h-screen bg-scroll ... ">
        ...
        <div class="w-full h-screen"style="background-image: url('/img/background.jpg');background-repeat:no-repeat;background-size:cover">
            <h1 class="font-bold text-5xl text-gray-800 flex justify-center ...  relative top-[7%] text-black italic font-serif ">投稿一覧画面(送信)</h1>
            <div class="w-3/4 mx-auto mt-8 bg-white p-5 relative top-[25%] ">
                 ...
                 @endphp
                    ...
                    @endphp
                        ...
                        <p>送信先：{{ $post->user->name}}</p>
                        <p>作成日：{{ $createdate->format('Y年m月d日')}}</p>
                        <p>送信予定日：{{ $senddate->format('Y年m月d日')}}</p>
                        @if($today < $senddate)
                        <p>あと{{$showdate}}日で送信される</p>
                        @endif
                        </a>
                    @endforeach
                </div>
